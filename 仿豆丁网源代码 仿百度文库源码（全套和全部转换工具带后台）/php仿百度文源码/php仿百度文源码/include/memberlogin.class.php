...
class MemberLogin
{
	...
	function __construct($kptime = -1)
	{
		...
		if($kptime==-1){
			...
			$this->M_KeepTime = 3600 * 1;//有效时间，1小时
		}else{
	...
	{
		global $dsql,$cfg_mb_feedcheck,$cfg_cmspath;
		//确定是否需要记录
		if (in_array($type,array('add','addsoft','feedback','addfriends','stow'))){
			...
			{
				$rcdtype = array('add'=>' 成功发布了', 'addsoft'=>' 成功发布了文档',
				                 'feedback'=>' 评论了文章','stow'=>' 收藏了');
				//内容发布处理
				$arcrul = " <a href='".$cfg_cmspath."/plus/view.php?aid=".$aid."'>".$title."</a>";
				$title = htmlspecialchars($rcdtype[$type].$arcrul, ENT_QUOTES);
			} else if ($type == 'addfriends')
			{
				//添加好友处理
				$arcrul = " <a href='".$cfg_cmspath."/member/index.php?uid=".$aid."'>".$aid."</a>";
