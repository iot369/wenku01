...
else if($dopost=="delArc")
{
	...
	if($rs)
	{
		...
		//扣除积分
		//scores=if((scores-5)<scores, (scores-5), 0)
		$dsql->ExecuteNoneQuery("Update `#@__member` set scores=if((scores-{$cfg_sendarc_scores})<scores, (scores-{$cfg_sendarc_scores}), 0) where mid='".$cfg_ml->M_ID."'; ");
