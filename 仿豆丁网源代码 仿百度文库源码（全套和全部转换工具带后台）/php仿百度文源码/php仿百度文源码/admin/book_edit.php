...
if($dopost!='save')
{
		...
		if(is_array($dtp->CTags))
		{
			foreach($dtp->CTags as $ctag)
			{
				if($ctag->GetName()=='link')
				{
					...
					$nForm .= "<div style='line-height:36px'>文档地址{$newRowStart}：<input type='text' name='softurl{$newRowStart}' style='width:280px' value='".trim($ctag->GetInnerText())."' />
	...
	$tags = GetTags($aid);
	include DedeInclude("templets/book_edit.htm");
	exit();
}
/*--------------------------------
function __save(){  }
...
else if($dopost=='save')
{
  ...
  if(empty($typeid2)) $typeid2 = 0;
	...
	$msg = "
    　　请选择你的后续操作：
    <a href='book_add.php?cid=$typeid'><u>发布新文档</u></a>
    &nbsp;&nbsp;
    <a href='archives_do.php?aid=".$id."&dopost=editArchives'><u>继续修改</u></a>
    &nbsp;&nbsp;
    <a href='$arcUrl' target='_blank'><u>查看文档</u></a>
    &nbsp;&nbsp;
