...
function lib_memberinfos(&$ctag,&$refObj)
{
	...
	$totaldocs = $dsql->GetOne("select count(1) totaldocs from t_archives where mid = '{$mid}'");

	while($row = $dsql->GetArray('mb'))
	{
		if($row['matt']==10) return '';
		$row['spaceurl'] = $GLOBALS['cfg_memberurl'].'/index.php?uid='.$row['userid'];
		$row['totaldocs'] = $totaldocs["totaldocs"];
		if(empty($row['face'])) {
