...
function htmldecode($text){
	...
	$text = str_replace("&#039;", "", $text);//去掉单引号
	return $text;
}

//文档缩略图处理，为空时返回默认缩略图地址
function docImageView($litpic){
	if($litpic == '-' || $litpic == ''){
		return $GLOBALS['cfg_cmspath'].'/images/defaultpic.gif';
	}
	return $litpic;
}
...
//added by czy 2010-10-03
function ShowMsgJson($code, $msg, $gourl,$onlymsg=0,$limittime=0)
{
	if(empty($GLOBALS['cfg_phpurl'])) $GLOBALS['cfg_phpurl'] = '..';
