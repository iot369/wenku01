...
if($step == 1)
{
	...
	if($dopost=='regbase')
	{
	   ...
	   '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
		if($dsql->ExecuteNoneQuery($inQuery))
		{
			...
			//写入其它默认数据
			//$dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','维软内容管理系统','http://www.uqbook.cn'); ");
			...
			if(!is_array($modelform))
			{
				...
				//更新今日用户注册 added by caozhiyang 2010-08-12
				doSysStatistics("reg_user");
	...
	if(!$cfg_ml->IsLogin())
	{
		...
		exit;
			...
			{
				 ShowMsg('你已经登陆系统，无需重新注册！', 'content_list.php?channelid=17');
				 exit;
			}
	}
