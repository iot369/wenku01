...
//根据文档ID获得文档类型filetype added by caozhiyang 2010-09-14
function GetFileTypeById($aid)
{
	global $dsql;
	//echo "Select filetype From `#@__addonbook` where aid='$aid'";
	$row = $dsql->GetOne("Select filetype From `#@__addonbook` where aid='$aid'");
	return $row['filetype'];
}
...
//获取某栏目下的文档数
function GetOneTypeDocNumber($typeid)
{
	global $dsql;
	//echo "Select count(1) docnumber From `#@__archives` where typeid='$typeid'";
	$row = $dsql->GetOne("Select count(1) docnumber From `#@__archives` where typeid='$typeid'");
