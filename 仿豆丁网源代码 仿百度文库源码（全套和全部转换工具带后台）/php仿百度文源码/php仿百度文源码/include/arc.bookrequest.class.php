...
class BookRequestList
{
	...
	//php4构造函数
	function BookRequestList($templet)
	{
		$this->__construct($templet);
	}
	...
	function CountRecord()
	{
		...
		{
			$cquery = "Select count(*) as dd From `#@__member_request`";
	...
	function ParseDMFields($PageNo,$ismake=1)
	{
		foreach($this->dtp->CTags as $tagid=>$ctag){
			...
			{
				$limitstart = ($this->PageNo-1) * $this->PageSize; //每页显示多少条
				if($limitstart<0)
				{
					$limitstart = 0;
				}
	...
	$imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
	{
		...
		$query = "Select a.*, b.userid, b.uname from t_member_request a, t_member b where a.mid=b.mid order by a.id desc limit $limitstart, $getrow";
