...
//-----------------------
//把上传的信息保存到数据库 by caozhiyang新增
//------------------------
function SaveBookUploadInfo($title,$filename,$medaitype=1,$filetype='',$addinfos='')
{
	global $dsql,$cfg_ml,$cfg_basedir;
	if($filename=='')
	{
	...
	$inquery = "INSERT INTO `#@__uploads`(title,url,mediatype,filetype,width,height,playtime,filesize,uptime,mid)
	   VALUES ('$title','$filename','$medaitype', '$filetype', '".$addinfos[0]."','".$addinfos[1]."','0','".$addinfos[2]."','$uptime','".$cfg_ml->M_ID."'); ";
	$dsql->ExecuteNoneQuery($inquery);
