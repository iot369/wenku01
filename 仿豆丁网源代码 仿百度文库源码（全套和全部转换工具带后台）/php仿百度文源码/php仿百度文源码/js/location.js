	...
	$.LocationSelect = {
		...
		build : function(user_opts) {
			...
			return $(this).each(function() {
				...
				specs.holder.append('<select name="province"><option value="0"code="0">省</option></select> &nbsp;&nbsp;<select name="city"><option value="0">市</option></select> &nbsp;&nbsp;<select name="district"><option value="0">地区</option></select>');
				try{
		        ...
		        if (!response || response.Status.code != 200) {
		            ...
		            $.each($.LocationSelect.data.city, function(i, item){
		                if (item.code.startsWith(specs._pCode.substring(0, 2))) {
		                    specs.selectors.city.append("<option value='" + item.value + "'>" + item.name + "</option>");
		...
		init: function(specs){
			...
			$.getJSON(specs.opts.data, function(data){
			    ...
			    $(specs.selectors.city).change(function(evt){
					value = $("select[name='city'] option[value="+evt.target.value+"]").attr('code');
			        fetch(value);
			    });
			    $(specs.selectors.province).change(function(evt){
					value = $("select[name='province'] option[value="+evt.target.value+"]").attr('code');
			        fetch(value);
			    });
			    $.each($.LocationSelect.data.province, function(i, item){
			        var str = "<option value='" + item.value + "' code='"+item.code+"'>" + item.name + "</option>";
		    ...
		    function fetch(target){
		        var _start, //leading string of code
					 ...
					 n = target;//value of option code in the former select element
		        ...
		        $.each(_result, function(i, item){
		            var str = "<option value='" + _result[i].value + "' code='"+item.code+"'>" + _result[i].name + "</option>";
		            $(_s).append(str);
		        });
